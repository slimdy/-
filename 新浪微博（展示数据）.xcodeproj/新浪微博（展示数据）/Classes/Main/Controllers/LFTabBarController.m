...
@property (nonatomic,weak)LFHomeTableViewController *home;
@property (nonatomic,weak)LFMessageTableViewController *message;
@property (nonatomic,weak)LFProfileTableViewController *profile;
@property (nonatomic,weak)LFDiscoveryTableViewController *discovery;
@end
/**
 *  总结：
 ...
 *     9.php 和 oc 都是if（0） 为假 其他的无论正数负数都为真
 */
...
- (void)viewDidLoad {
    ...
    //设置tabbar
    [self setTabBar];
    //设置一个定时器，每2秒 获取一次用户的未读数据
    self.timer = [NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(requestUnread) userInfo:nil repeats:YES];
}
/**
 *  设置用户的未读数据在badgeView 上的显示
 */
-(void)requestUnread{
    //运用自己定义的用户工具类获得数据
    [LFUserTool getUnreadWithSuccess:^(LFRequestUnreadResultModel *resultModel) {
        
        // 设置首页未读数
        _home.tabBarItem.badgeValue = [NSString stringWithFormat:@"%d",resultModel.status];
        
        // 设置消息未读数
        _message.tabBarItem.badgeValue = [NSString stringWithFormat:@"%d",resultModel.messageCount];
        
        // 设置我的未读数
        _profile.tabBarItem.badgeValue = [NSString stringWithFormat:@"%d",resultModel.follower];
...
-(void)tabBar:(LFTabBar *)tabBar didClickBtn:(NSInteger)index{
    //如果tabbar 点击了 首页按钮，并且之前就在首页上，则刷新首页
...
-(void)tabBar:(LFTabBar *)tabBar didClickPlustBtn:(UIButton *)plusBtn{
    LFComposeViewController *composeVC = [[LFComposeViewController alloc]init];
    composeVC.view.backgroundColor = [UIColor whiteColor];
    LFNavigationController *composeNAV = [[LFNavigationController alloc]initWithRootViewController:composeVC];
